...
class DownloadValidationError(RuntimeError):
    """A non-transient response or integrity failure that should not be retried."""
            ...
            try:
                ...
                with urlopen(request, timeout=timeout_seconds) as response, open(tmp_path, 'wb') as out_file:
                    ...
                    if max_byte_count > 0 and total_size is not None and total_size > max_byte_count:
                        raise DownloadValidationError(f'Download is larger than the allowed limit. Limit {max_byte_count}, got {total_size}.')
                    if expected_byte_count > 0 and total_size is not None and total_size != expected_byte_count:
                        raise _DownloadValidationError(
                            f'Downloaded size mismatch. Expected {expected_byte_count}, got {total_size}.'
                        )
                    downloaded = 0
                    while True:
                        _ensure_not_stopped(stop_event, message=stop_message)
                        ...
                        if max_byte_count > 0 and downloaded > max_byte_count:
                            raise DownloadValidationError(f'Download exceeded the allowed limit of {max_byte_count} bytes.')
                ...
                if expected_byte_count > 0 and actual_size != expected_byte_count:
                    raise DownloadValidationError(f'Downloaded size mismatch. Expected {expected_byte_count}, got {actual_size}.')
                if expected_digest:
                    actual_digest = sha256_file(tmp_path)
                    if actual_digest != expected_digest:
                        raise DownloadValidationError('SHA256 verification failed for downloaded file.')
                ...
                if tmp_path.exists():
                    tmp_path.unlink()
                raise DownloadValidationError(f'{error_prefix}: {exc}') from exc
                ...
                if attempt < max(1, int(retries_per_header)):
                    ...
                    if stop_event is None:
                        ...
                        if tmp_path.exists():
                            tmp_path.unlink()
                        raise InterruptedError(stop_message)
    if tmp_path.exists():
        tmp_path.unlink()
